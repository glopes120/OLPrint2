...
export const Support: React.FC<SupportProps> = ({ onOpenChat, orders }) => {
  ...
  const renderStatusBadge = (status: string) => {
    switch(status) {
      case 'Entregue':
        return <span className="px-3 py-1 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle className="h-3 w-3" aria-hidden="true" /> {status}</span>;
      case 'Enviado':
      case 'Em Distribuição':
        return <span className="px-3 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-xs font-bold flex items-center gap-1"><Truck className="h-3 w-3" aria-hidden="true" /> {status}</span>;
      case 'Em Processamento':
        return <span className="px-3 py-1 bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-full text-xs font-bold flex items-center gap-1"><Clock className="h-3 w-3" aria-hidden="true" /> {status}</span>;
      case 'Cancelado':
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pb-20 animate-in fade-in duration-500">
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-12 relative z-20">
        ...
        {/* Navigation Tabs */}
        <div className="bg-white dark:bg-slate-900 rounded-t-2xl border-b border-slate-200 dark:border-slate-800 shadow-sm flex overflow-hidden" role="tablist" aria-label="Navegação de suporte">
          <button 
        ...
        <div className="bg-white dark:bg-slate-900 rounded-b-2xl shadow-lg border border-slate-200 dark:border-slate-800 border-t-0 p-6 md:p-8 min-h-[500px]">
          ...
          {/* --- TAB: HELP CENTER --- */}
          {activeTab === 'help' && (
            <div id="panel-help" role="tabpanel" aria-labelledby="tab-help" className="animate-in fade-in slide-in-from-left-4 duration-300">
              
              {/* Quick Cards */}
              <div className="grid md:grid-cols-3 gap-6 mb-12">
                 <div 
                   className="p-6 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 hover:border-blue-200 dark:hover:border-blue-800 transition-all group cursor-pointer" 
                   onClick={() => setActiveTab('orders')}
                   role="button"
                   tabIndex={0}
                 ...
                 <div 
                    className="p-6 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 hover:border-purple-200 dark:hover:border-purple-800 transition-all group cursor-pointer" 
                    onClick={onOpenChat}
                    role="button"
                    tabIndex={0}
              ...
              <div className="grid lg:grid-cols-2 gap-12">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                    <ShieldCheck className="text-blue-600" aria-hidden="true" /> Perguntas Frequentes
                  </h2>
                    ...
                    {faqs.map((faq, index) => (
                      ...
                      >
                        ...
                        {activeFaq === index && (
                          <div 
                            id={`faq-answer-${index}`}
                            className="px-6 pb-4 text-slate-600 dark:text-slate-400 text-sm leading-relaxed animate-in slide-in-from-top-2 border-t border-slate-50 dark:border-slate-800 pt-4"
                          >
                            {faq.answer}
                          </div>
                        )}
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-2xl border border-slate-200 dark:border-slate-800">
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                    <Mail className="text-blue-600" aria-hidden="true" /> Envie-nos uma mensagem
                  </h2>
                  
                  {formSubmitted ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-10 animate-in fade-in zoom-in" role="alert">
                    ...
                    <form onSubmit={handleContactSubmit} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label htmlFor="contact-name" className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase">Nome</label>
                          <input id="contact-name" required type="text" className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                        <div>
                          <label htmlFor="contact-email" className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase">Email</label>
                          <input id="contact-email" required type="email" className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                      </div>
                      <div>
                        <label htmlFor="contact-subject" className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase">Assunto</label>
                        <select id="contact-subject" className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none">
                      ...
                      <div>
                        <label htmlFor="contact-message" className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase">Mensagem</label>
                        <textarea id="contact-message" required rows={4} className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none"></textarea>
                      </div>
          ...
          {/* --- TAB: MY ORDERS --- */}
          {activeTab === 'orders' && (
             <div id="panel-orders" role="tabpanel" aria-labelledby="tab-orders" className="animate-in fade-in slide-in-from-right-4 duration-300">
               ...
               <div className="space-y-4">
                   ...
                   <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 hover:shadow-md transition-shadow">
                      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                         <div className="flex items-center gap-4">
                            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-blue-600 dark:text-blue-400" aria-hidden="true">
                            ...
                            <div>
                               ...
                               <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2 mt-1">
                                 <Clock className="h-3 w-3" aria-hidden="true" /> {order.date}
                               </p>
                            </div>
                         </div>
                         <div className="text-left md:text-right">
                            <p className="text-sm text-slate-500 dark:text-slate-400">Total</p>
                            <p className="text-xl font-bold text-slate-900 dark:text-white" aria-label={`Total: ${order.total.toFixed(2)} euros`}>€{order.total.toFixed(2)}</p>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            ...
                            <ul className="text-sm text-slate-700 dark:text-slate-300 space-y-1">
                              {order.items.map((item, i) => (
                                <li key={i} className="flex items-center gap-2">
                                  <div className="w-1.5 h-1.5 bg-slate-300 rounded-full" aria-hidden="true"></div>
                                  {item}
                                </li>
                              ))}
                            </ul>
                         ...
                         <div className="flex gap-3 w-full md:w-auto">
                            ...
                            <button className="md:hidden px-3 py-2 text-slate-400" aria-label="Ver detalhes">
                 ...
                 {displayedOrders.length === 0 && (
                    <div className="text-center py-12">
                       <Package className="h-12 w-12 mx-auto text-slate-300 mb-3" aria-hidden="true" />
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-6 text-center border-t border-slate-200 dark:border-slate-800 pt-12">
          <div className="p-4">
            <Phone className="h-8 w-8 mx-auto text-slate-400 mb-3" aria-hidden="true" />
          ...
          <div className="p-4">
            <Mail className="h-8 w-8 mx-auto text-slate-400 mb-3" aria-hidden="true" />
          ...
          <div className="p-4">
            <MapPin className="h-8 w-8 mx-auto text-slate-400 mb-3" aria-hidden="true" />
